...
- [ ] Reuse existing canonical schemas in `src/types/recipe.ts` for all generation inputs/outputs.
- [ ] Map saved profile fields into `BeanProfileSchema` keys (`bean_name`, `variety`, `roast_level`, etc.) instead of introducing alternate naming.
- [ ] Restrict brewer options to `MethodIdSchema` values only (`v60`, `origami`, `orea_v4`, `hario_switch`, `kalita_wave`, `chemex`, `ceado_hoop`, `pulsar`, `aeropress`).
- [ ] Restrict goals to `BrewGoalSchema` values only (`clarity`, `balanced`, `sweetness`, `body`, `forgiving`).
- [ ] Reuse existing generation pipeline (`/api/generate-recipe`) and existing save pipeline semantics (`/api/recipes`) rather than creating parallel incompatible contracts.
...
- [ ] Add unique constraint: `unique (id, user_id)` for composite FK ownership enforcement.
- [ ] Add indexes:
...
- [ ] Add columns:
  ...
  - `created_at timestamptz not null default now()`
- [ ] Add FK `(coffee_profile_id, user_id)` -> `coffee_profiles(id, user_id)` to prevent cross-user drift.
- [ ] Add partial unique index for one primary image per profile in v1:
  - `unique (coffee_profile_id) where is_primary = true`

### 2.3 Extend existing `recipes`
- [ ] Add nullable linkage columns:
  - `coffee_profile_id uuid`
  - `coffee_profile_user_id uuid`
- [ ] Add generation provenance column:
  - `generation_context jsonb null` with shape:
    - `{ source: 'profile', goal, water_mode, water_grams?, water_delta_grams?, method }`
- [ ] Add composite FK `(coffee_profile_id, coffee_profile_user_id)` -> `coffee_profiles(id, user_id)`.
- [ ] Keep immutable snapshot requirement: recipe creation still copies bean into `recipes.bean_info` and snapshot tables.

## 3. RLS + Integrity Rules
- [ ] `coffee_profiles` RLS: user can CRUD only rows where `user_id = auth.uid()`.
- [ ] `coffee_profile_images` RLS: user can CRUD only rows where `user_id = auth.uid()`.
- [ ] `recipes` linkage writes must preserve same-owner composite FK.
...
- [ ] Implement authenticated multipart route handler for profile image upload.
- [ ] Server-side optimization pipeline:
  ...
  - strip metadata/EXIF
- [ ] Insert DB metadata only after successful storage upload.
- [ ] On DB insert failure after upload, delete uploaded object immediately.
- [ ] On image replacement, delete old storage object after successful swap.
- [ ] On profile hard delete, delete all associated storage objects then DB rows.

## 5. API Contracts (Aligned)

### 5.1 `POST /api/coffee-profiles`
- [ ] Auth required.
- [ ] Accept scan-derived bean payload mapped to `BeanProfileSchema` + `label`.
- [ ] Create profile row.
- [ ] Optional multipart image in same flow or follow-up upload endpoint.
- [ ] Return `{ profile }`.

### 5.2 `GET /api/coffee-profiles`
- [ ] List active profiles (`archived_at is null` by default), paginated.
- [ ] Include primary image signed URL.

### 5.3 `GET /api/coffee-profiles/:id`
- [ ] Return profile, image metadata/url, and recent linked recipes.

### 5.4 `PATCH /api/coffee-profiles/:id`
- [ ] Update label and editable bean fields (validated through mapped `BeanProfileSchema`).

### 5.5 `POST /api/coffee-profiles/:id/archive`
- [ ] Set `archived_at`.
- [ ] Archived profile remains readable for history views.

### 5.6 `POST /api/recipes/from-profile`
- [ ] Auth required.
- [ ] Validate profile ownership and `archived_at is null`.
...
- [ ] Build generation request by combining profile `bean_profile_json` and override inputs.
- [ ] Call existing generation logic.
- [ ] Persist immediately as recipe via existing save semantics, including snapshots.
- [ ] Set recipe linkage (`coffee_profile_id`, `coffee_profile_user_id`) and `generation_context`.
- [ ] Update profile `last_used_at`.

## 6. UI Scope (v1)
- [ ] Saved Coffees list page with thumbnail, label, roaster, roast level, last used.
- [ ] Profile detail view with image + bean data + Generate form.
- [ ] Generate form fields:
  - method selector (from canonical method list)
  - goal selector (from canonical goal list)
  - water mode + numeric input
- [ ] Scan completion flow includes save confirmation/edit step.
- [ ] Archive action available from profile detail.
- [ ] Prevent generation from archived profiles with explicit user message.

## 7. Migration + Rollout
- [ ] Net-new only: do not backfill old recipes/images.
- [ ] Add migrations for tables, indexes, FKs, RLS, and `recipes` new columns.
- [ ] Add feature gating only if project already has an existing mechanism; otherwise ship behind environment flag check in routes/UI.
...
## 9. Sequencing Plan
- [ ] Phase 1: DB migration + RLS + composite FK integrity
- [ ] Phase 2: Storage bucket + upload/optimization route + cleanup paths
- [ ] Phase 3: profile CRUD APIs + list/detail UI
- [ ] Phase 4: `/api/recipes/from-profile` integration with existing generation/save flow
- [ ] Phase 5: tests, staging verification, rollout

## 10. Risks + Mitigations
- [ ] Risk: schema drift from existing generation types.
  - Mitigation: enforce mapping layer + zod validation against canonical schemas.
- [ ] Risk: orphaned storage objects.
  - Mitigation: explicit cleanup on failure/replacement/delete.
- [ ] Risk: ownership mismatch across linked tables.
