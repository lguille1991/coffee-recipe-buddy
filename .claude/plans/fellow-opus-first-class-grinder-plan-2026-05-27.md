...
- Task-owned files:
  ...
  - `docs/migration_004_add_fellow_opus.sql`
  ...
  - test files covering grinder conversion, migrations, validation, profile/settings, and saved-recipe editing flows
...
- [ ] Expand the grinder domain model to include `fellow_opus`.
...
- [ ] Add profile persistence support for `fellow_opus`.
  - Create a new SQL migration doc to extend the `profiles.preferred_grinder` check constraint with `fellow_opus`.
...
- [ ] Add deterministic Fellow Opus conversion and formatting helpers in `src/lib/grinder-converter.ts`.
...
- [ ] Extend derived grind-setting generation to emit `fellow_opus`.
...
- [ ] Update recipe creation and editing flows to understand Opus as a decimal grinder.
...
- [ ] Add automatic recipe migration for `fellow_opus`.
...
- [ ] Update UI surfaces that render grinder choices or secondary grinder recommendations.
...
- [ ] Update generation/validation documentation and contracts to five grinders.
