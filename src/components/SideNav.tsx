  ...
  return (
    <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-56 bg-[var(--card)] border-r border-[var(--border)] z-40">
      ...
      <div className="px-6 py-8 border-b border-[var(--border)]">
        ...
        <p className="text-sm text-[var(--muted-foreground)] mt-0.5">Coffee Recipe Buddy</p>
      </div>
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-[12px] text-base font-medium transition-colors ${
                active
